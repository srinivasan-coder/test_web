...
/** Replaces one video testimonial's poster image. Always required, so no delete — matches Cover on galleries. */
export function VideoTestimonialPosterUploader({
  videoTestimonialId,
  initialSrc,
}: {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const signRes = await fetch("/api/admin/upload/video-testimonial-sign", {
        ...
        body: JSON.stringify({ videoTestimonialId, field: "poster", mimeType: file.type }),
      });
      ...
      const finalizeRes = await fetch("/api/admin/upload", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ videoTestimonialId, field: "poster", url, publicId: signed.publicId }),
      });
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-background">
      ...
      <div className="p-4">
        <p className="text-sm font-medium text-foreground">Poster</p>
