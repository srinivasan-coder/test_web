...
/**
 * Replaces one photo (cover or an existing images[] entry) on a single
 * gallery — the gallery-edit equivalent of SlotUploader
 * (components/admin/slot-uploader.tsx), keyed by galleryId+field instead of
 * the static admin-sections section+slotId.
 */
export function GalleryPhotoUploader({
  ...
  label: string;
  initialSrc: string;
  /** Present for an additional photo (not the cover, which can't be removed). */
  deleteIndex?: number;
}) {
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const signRes = await fetch("/api/admin/upload/gallery-sign", {
        ...
        body: JSON.stringify({ galleryId, field, mimeType: file.type }),
      });
      ...
      const finalizeRes = await fetch("/api/admin/upload", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ galleryId, field, url, publicId: signed.publicId }),
      });
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/admin/content/galleries/${galleryId}/photos/${deleteIndex}`, {
      method: "DELETE",
    });
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-background">
      <div className="relative aspect-[4/3] bg-secondary">
        ...
        {deleteIndex !== undefined && !confirmingDelete && (
          <button
            type="button"
