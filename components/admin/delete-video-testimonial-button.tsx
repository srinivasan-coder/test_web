...
/** Same icon-overlay + confirm-scrim pattern as DeleteGalleryButton / DeleteInstagramButton. */
export function DeleteVideoTestimonialButton({ id, title }: { id: string; title: string }) {
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/admin/content/video-testimonials/${id}`, { method: "DELETE" });
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-ink/85 p-4 text-center backdrop-blur-sm">
      <p className="text-xs text-white">
        Delete <span className="font-medium">{title}</span>? It disappears from the Reviews page
